...
const BASE_URL = {
  development: 'https://india-loan-test.6688-epoch.com',
  production: 'https://vfekmp.online'
}

// create an axios instance
const service = axios.create({
  // baseURL: process.env.API_BASE_API, // url = base api url + request url
...
function unzip(b64Data) {
  ...
  for (i = 0; i < array.length / maxRange; i++) {
    result += String.fromCharCode.apply(
      null,
      array.slice(i * maxRange, (i + 1) * maxRange)
    )
  }
...
service.interceptors.request.use(
  (config) => {
    ...
    config.headers['channelCode'] =
      store.getters.appGlobal.channelCode || 'google'
    config.headers['mobileType'] = store.getters.appGlobal.mobileType || '2'
    config.headers['gps'] = store.getters.appGlobal.gps
    config.headers['gpsAddress'] = encodeURIComponent(
      store.getters.appGlobal.gpsAddress
    )
  ...
  (response) => {
    ...
    if (res.returnCode && res.returnCode !== 2000) {
      ...
      if (res.returnCode === 4005 || res.returnCode === 4006) {
        try {
          wjs['codeDispose'](
            JSON.stringify({ code: res.returnCode, msg: res.message })
          )
        } catch (error) {
